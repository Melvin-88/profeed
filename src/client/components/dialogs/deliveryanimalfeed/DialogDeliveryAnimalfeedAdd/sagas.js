...
function* submitAddDeliveryAnimalfeed(action) {
  const { deliveryAnimlfeed } = action.payload;

  try {
    yield call(serverApi.addDeliveryAnimalfeed, deliveryAnimlfeed);
...
export default function* mainSaga() {
  yield takeEvery(actions.DIALOG_DELIVERY_ANIMALFEED_SUBMIT, submitAddDeliveryAnimalfeed);
}
